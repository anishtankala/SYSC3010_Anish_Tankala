...
# If it doesn’t work, get your own.
...
data = json.loads(results)# Print the results
current = data["main"]
humidity = data["main"]

cursor.execute('''insert into weather(city, humidity) values (?, ?)''', (city, humidity["humidity"]))
dbconnect.commit();
